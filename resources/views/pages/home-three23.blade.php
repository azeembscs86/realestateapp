...
<div class="row">
<ul class="submenu col-md-4 col-sm-6 col-xs-12 pull-left padding-right-0">
<?php $count=0; 
function cmp($a, $b)
{
...
<ul class="submenu col-md-4 col-sm-12 pull-left special-margin padding-right-0">
<?php } ?>
<?php } ?>
</ul>
</div>
...
<label for="radio100"> No Garage </label>
</div>
<div class="radio my-radio radio-primary radio-inline">
...
@else
<img class="img-responsive b-lazy {{$property->images->first()->image_class}}" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="{{asset('pictures/placeholder.png')}}" alt="property for sale by owner">
@endif
<div class="flat-link"> 
...
<span class="price"><?php echo $_COOKIE['user_currency'].'-'.EnvatoUser::calculateCurrency($property->currency_code,$_COOKIE['user_currency'],$property->sale_price); ?></span>
<?php }else{ ?>	
...
<p>MatchPropertyDirect mission is to match buyers and sellers directly worldwide via the internet.</p></div>
...
<script>
    ...
    method: "POST",
		...
		'<div id="siteNotice">'+
		'</div>'+
		'<div id="bodyContent" class="map-body">'+'<img src="http://107.6.184.93/MatchPropertyDirect/'+properties[i]['propertyimage']+'" width="250" height="120"/>'+
		'<a target="_blank" href="http://107.6.184.93/MatchPropertyDirect/show/'+properties[i]['slug']+'")}}><h4 id="firstHeading" class="firstHeading">'+properties[i]['title']+'</h4></a>' +
		'</div>'+
		'</div>'+
		'</div>'; 
